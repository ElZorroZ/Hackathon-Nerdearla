...
export function SelectTrigger({ className, children }: SelectTriggerProps) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative z-10 flex w-full items-center justify-between gap-2 rounded-xl border border-border bg-popover px-3 py-2 text-sm text-foreground outline-none transition-colors",
...
export function SelectContent({ className, children }: SelectContentProps) {
  ...
  return (
    <motion.div
      ...
      aria-hidden={!open}
      {...({ inert: !ctx.open ? "" : undefined } as Record<string, unknown>)}
      initial={false}
      animate={
        ctx.reduce
      ...
      className={cn(
        "absolute left-0 right-0 z-20 rounded-xl border border-border bg-popover shadow-lg",
